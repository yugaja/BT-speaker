...
void Config::setUrl(const char* station) {
  memset(config.station.url, 0, BUFLEN);
  strlcpy(config.station.url, station, BUFLEN);
  u8fix(config.station.url);
  display.putRequest(NEWARTIST);
}
...
void Config::bootInfo() {
  ...
  BOOTLOG("ir:\t\t%d", IR_PIN);
  BOOTLOG("mode:\t%d", store.play_mode);  
