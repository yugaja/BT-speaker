...
void controlsEvent(bool toRight, int8_t volDelta) {
 ...
 // --- Added friendly pause on wrap-around ---
    if (p < 1) {
      // Short pause before wrapping from start to end
  //    delay(400); // 400 ms is enough for "pause" feeling
      p = cs;     // wrap to last item
    }
    else if (p > cs) {
      // Short pause before wrapping from end to start
  //    delay(400);
      p = 1;      // wrap to first item
    }
