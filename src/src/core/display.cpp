...
void Display::_title() {
    Serial.print("[DISPLAY][TITLE] raw: ");
    Serial.println(config.station.title);
...
void Display::_artist() {
    Serial.print("[DISPLAY][ARTIST] raw: ");
    Serial.println(config.station.url);

    if (strlen(config.station.url) > 0) {
        _title2->setText(config.station.url);
    } else {
        _title2->setText("");
    }
}
