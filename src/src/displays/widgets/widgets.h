...
class VuWidget: public Widget {
  public:
    VuWidget() {}// MÃ³dosÃ­tÃ¡s: vumidcolor plussz paramÃ©ter.
    ...
    void loop();
    static void setLabelsDrawn(bool value); // MÃ³dosÃ­tÃ¡s
    static bool isLabelsDrawn(); // MÃ³dosÃ­tÃ¡s
    void _clear();    
  protected:
    #if !defined(DSP_LCD) && !defined(DSP_OLED)
      Canvas *_canvas;
    #endif
    static bool   _labelsDrawn; // MÃ³dosÃ­tÃ¡s Ãºj vÃ¡ltozÃ³.
    VUBandsConfig _bands;
    uint16_t _vumaxcolor, _vumidcolor, _vumincolor; // MÃ³dosÃ­tÃ¡s: plussz _vumidcolor
    void _draw();
};
